...
def verify_45_rule(all_students, student_matching, groups):
    ...
    rational_groups = task5.get_rational_group_students(all_students, groups)
    ...
    for school, students_school in school_matching.items():
        rational_school = task5.get_rational_group_students(students_school, groups)
        for g in groups:
            result[g][school.school_parent] = rational_school[g] / rational_groups[g] if rational_groups[g] != 0.0 else None
    json = {}
    for g, schools_rationals in result.items():
        json[g] = dict(schools_rationals)
    return json
...
def task6():
    ...
    # instance2
    print('--------------------')
    matching_table, schools, students = instance2.generate_matching_table_schools_and_students(20)
    ...
    # instance3
    print('--------------------')
    matching_table, schools, students = instance3.generate_matching_table_schools_and_students(20)
