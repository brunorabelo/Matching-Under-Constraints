...
def generate_matching_table_schools_and_students(n):
    quota_group = {g: (0.9 * n) // 4 for g in GROUPS}
    s1 = School('s1', quota_group=quota_group, capacity=n // 4)
    s2 = School('s2', quota_group=quota_group, capacity=n // 4)
    schools = [s1, s2]
    students = []
    ...
    for i in range(pa + pb, pa + pb + pc):
        students.append(Student(f'i{i}', "C"))
    for i in range(pa + pb + pc, n):
        students.append(Student(f'i{i}', "D"))
