...
def next_school_for_student(matching_table, schools, students, matched_schools, target_student):
    ...
    for school in preferred_schools:
        student_group = students[target_student]
        max_quantity_group_school = schools[school][student_group]
        current_quantity_group_school = 0
        for sc, st in matched_schools.items():
            if str(sc).startswith(school[:-1]) and student_group == students[st]:
                current_quantity_group_school += 1

        if current_quantity_group_school==max_quantity_group_school and student_group:
            pass
...
def task3():
    ...
    schools = {
        's1': {'quota_group': {"A": 2, "B": 2}, "capacity": 2},
        's2': {'quota_group': {"A": 2, "B": 2}, "capacity": 2}
    }
